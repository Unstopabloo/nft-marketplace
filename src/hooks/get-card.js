import { ETHEREUM_PRICE } from './api.js'
...
// Llama a esta función antes de realizar la consulta para mostrar el loader
function startQuery() {
  showLoader()
}

// Llama a esta función después de que se complete la consulta para ocultar el loader
function endQuery() {
  hideLoader()
}
...
async function getCards(query, page) {
  try {
    ...
    const ethPrice = await ETHEREUM_PRICE()

    $.post(`./src/db/queries.php?${query}&page=${page}`, (data) => {
      data = $.parseJSON(data)

      $('#founds').html(`Encontrados: ${data.length}`)
        ...
        const card = `
          <div class="col-12 col-sm-6 col-md-4 col-lg-3 d-flex justify-content-center py-1 mb-5">
            <a href="#" data-id=${value.id} type="button" data-bs-toggle="modal" data-bs-target="#exampleModal" class="openModal product-1 px-2">
              ...
              <img class="col-12 product-card-img" src="./assets/img/${value.sku}.webp" alt="sadasd">
      ...
      $('.product-1').on('click', function () {
        $.post('./src/db/queries.php?card=modal', {
          ...
          const modalBody = `
                  ...
                  <div class="row">
                    ...
                    <div class="modal-product-content col-md-6 col-12">
                      <div class="modal-header row">
                        <span class="modal-artist col-11">${
                          data.artistas_id
                        }</span>
            ...
            data.precio * Math.floor(ethPrice)
          )} USD</strong>
                      </div>
...
let currentFilter = 'card=get_cards'

$(document).ready(async () => {
  currentPage = 1
  getCards(currentFilter, currentPage)
})

$('#by-menor').on('click', async () => {
  currentFilter = 'card=get_cards_by_menor'
  currentPage = 1 // Reiniciar la página al cambiar el filtro
  getCards(currentFilter, currentPage)
})

$('#by-mayor').on('click', async () => {
  currentFilter = 'card=get_cards_by_mayor'
  currentPage = 1 // Reiniciar la página al cambiar el filtro
  getCards(currentFilter, currentPage)
})

$('#by-alf').on('click', async () => {
  currentFilter = 'card=get_cards_by_alf'
  currentPage = 1 // Reiniciar la página al cambiar el filtro
  getCards(currentFilter, currentPage)
})

$('#clear').on('click', async () => {
  currentFilter = 'card=get_cards'
  currentPage = 1 // Reiniciar la página al cambiar el filtro
  getCards(currentFilter, currentPage)
})
...
$('#input-filter').on('input', function () {
  ...
  getCards('card=get_cards_by_search&searchQuery=' + searchQuery, currentPage)
})
